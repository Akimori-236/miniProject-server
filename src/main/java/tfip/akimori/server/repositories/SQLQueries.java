...
public interface SQLQueries {
    ...
    public final String SQL_GETSTOREBYEMAIL = """
            SELECT instrument_id, instrument_type, brand, model, serial_number, store_name, u2.firstname, u2.lastname, u2.email
            FROM instruments i
            INNER JOIN store s
    ...
    public final String SQL_GETINSTRUMENTSBYEMAIL = """
            SELECT instrument_id, instrument_type, brand, model, serial_number, store_name, firstname, lastname, email
            FROM instruments i
            INNER JOIN store s
            ON i.store_id = s.store_id
            INNER JOIN users u
